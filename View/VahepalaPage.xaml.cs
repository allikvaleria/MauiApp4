...
public partial class VahepalaPage : ContentPage
{
    ...
    public void SelgeVorm()
    {
        ...
        vahepalaListView.SelectedItem = true;
        btn_kustuta.IsVisible = false;
        fotoSection.Clear();
    }
    private void Btn_hide_Clicked(object sender, EventArgs e)
    {
        // Показываем tableview и скрываем список, или наоборот
        bool showTableView = !tableview.IsVisible;

        tableview.IsVisible = showTableView;
        vahepalaListView.IsVisible = !showTableView;

        // Обновляем текст кнопки
        btn_hide.Text = showTableView ? "Näita loendit" : "Näita sisestust";
    }
